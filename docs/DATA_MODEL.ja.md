...
`demo`（`{ active, startedAt }`）はデモモードを示します。デモ開始時は学習者のフルバックアップを localStorage の
`neonsec-academy:pre-demo-backup` に退避し、決定的な合成データセットをインポートします。終了時は退避したバックアップを
復元してキーを削除します。バックアップを退避できない場合、デモモードは開始しません。
