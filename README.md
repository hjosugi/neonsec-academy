...
| CEH+ Tracks | Beyond District hub for pentest, AppSec, cloud, SOC, IR, and threat-model tracks. The pentest track includes a guided engagement workflow (scope quiz, RoE, asset inventory, finding triage, Markdown report); the AppSec track has 10 code review challenges (select the vulnerable line, classify, write impact and fix, see a safe fix and unit-test idea) with weakness stats and review-queue hand-off; the cloud track has 8 synthetic config reviews with least-privilege explanations and cloud weakness stats; the SOC track has 10 synthetic log investigations with a timeline builder, detection logic, and containment ideas; the IR track builds an incident timeline (time, source, observation, confidence, evidence) and a Markdown IR report with a quality checklist; the threat-model track has 5 fictional architectures where STRIDE threats and mitigations become a remediation backlog for the Report Builder. |
...
| Portfolio | Public-safe Markdown portfolio from mock exam, practical, track, report, finding, and reflection data, with a sensitive placeholder check and a required privacy checklist. |
| Interview Prep | Skill evidence per CEH domain and CEH+ track (concepts, labs, reports, challenges), honest gaps with next plans, STAR/concise stories, and English answer memos with speaking-time estimates; stories flow into the portfolio. |
...
- Lab pack authoring format and import pipeline: [docs/LAB_PACK_FORMAT.md](docs/LAB_PACK_FORMAT.md)
