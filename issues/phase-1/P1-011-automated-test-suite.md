...
- [ ] Test `lib/srs.ts` scheduling (grade → interval/ease/lapse transitions, due calc)
- [ ] Test `lib/grade.ts` (mcq/multi/true_false correctness, edge cases)
- [ ] Test `lib/exam.ts` weighting (counts match blueprint, shortfall redistribution, grading)
- [ ] Test `lib/analytics.ts` (module/domain mastery, weakest selection)
- [ ] Test `lib/readiness.ts` banding
